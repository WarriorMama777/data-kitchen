...
def process_image(args):
    image_path, save_path, background, resize, format, quality, debug = args
    if debug:
...
def process_directory(dir_path, save_dir, extensions, recursive, background, resize, format, quality, debug, threads):
    if debug:
    ...
    args = [(file_path, save_dir / Path(str(file_path.relative_to(dir_path)).rsplit('.', 1)[0]), background, resize, format, quality, debug) for file_path in files]

    with ThreadPoolExecutor(max_workers=threads) as executor:
        list(tqdm(executor.map(process_image, args), total=len(args), desc="Processing images"))
...
def main():
    ...
    parser.add_argument('--threads', type=int, default=4, help='Number of threads to use for processing images')
