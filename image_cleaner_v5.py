...
def parse_arguments():
    ...
    parser.add_argument('--process_group_overlay', type=int, default=0, help='Number of overlapping images between process groups')
...
def remove_duplicates(image_files, threshold, save_dir, save_dir_duplicate, debug, mem_cache, args):
    ...
    process_group_overlay = args.process_group_overlay  # process_group_overlayの値を取得

    # 画像ファイルをprocess_groupとprocess_group_overlayの数に応じて分割
    grouped_image_files = []
    for i in range(0, len(image_files), process_group - process_group_overlay):
        grouped_image_files.append(image_files[i:i + process_group])
...
def process_duplicate_save(duplicate, original, save_dir_duplicate, args):
    relative_path = os.path.relpath(duplicate, start=os.path.commonpath([duplicate, args.dir]))
    if args.preserve_structure or args.preserve_own_folder:
