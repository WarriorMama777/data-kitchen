...
def main():
    ...
    if args.recursive:
        imagePathList = [path for path in paths.list_images(args.dir) if path.split('.')[-1].lower() in allowed_extensions]
    else:
        imagePathList = [os.path.join(args.dir, f) for f in os.listdir(args.dir) if f.split('.')[-1].lower() in allowed_extensions]
